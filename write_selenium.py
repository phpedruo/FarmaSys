...
class Teste_02_Navegacao(BaseTestCase):
    def test_04_deve_carregar_pagina_inicial(self):
        self.abrir_pagina("/")
        self.wait.until(EC.presence_of_element_located((By.TAG_NAME, "body")))
        self.assertEqual(self.driver.title, self.driver.title)
    ...
    def test_06_deve_carregar_pagina_de_ofertas(self):
        self.abrir_pagina("/ofertas/")
        self.wait.until(EC.presence_of_element_located((By.TAG_NAME, "body")))
        self.assertIn("ofertas", self.driver.current_url.lower())
"""

with open("farmacia/tests_selenium.py", "w", encoding="utf-8") as f:
    f.write(content)
